...
- **Acceptance Criteria:**
  ...
  - [ ] Given an export rule of type `note`, When the rule fires, Then exactly the configured note path is exported if it still exists; otherwise the run is recorded as a rule-level failure with `errorCode: source-not-found`. (`flattenOnTarget` is irrelevant for single-note rules.)
