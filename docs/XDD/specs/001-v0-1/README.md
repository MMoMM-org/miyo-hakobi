...
| 2026-05-02 | Spec gap closures: Gap 1 (folder picker) and Gap 2 (rule picker) wired in production. | Gap 1: `chooseFsFolder` in `src/main.ts` now resolves Electron's `dialog.showOpenDialog` via the renderer's runtime `require` (`electron.remote.dialog` first, `@electron/remote` fallback), with a Notice fall-through if neither is available — closing the prior `TODO (Phase 4)` no-op. Gap 2: new `src/ui/RulePickerModal.ts` (FuzzySuggestModal<Rule>) is injected into `CommandRegistry` as the `selectRule` seam, so the four "Run an import/export rule…" + "Dry-run …" commands present a real fuzzy picker instead of silently no-op'ing. Mock extension: `test/__mocks__/obsidian.ts` gains a minimal subclass-friendly `FuzzySuggestModal` stub. Tests added: `test/ui/RulePickerModal.test.ts`. The existing F7/`compliance.test.ts` coverage continues to pass unchanged. |
| 2026-05-02 | T4.7 honest defer: F12 (file-menu integration, Could-Have) recorded as `DEFERRED:file-menu-integration` sentinel in the F# → test mapping. | The previous mapping pointed F12 at `test/ui/CommandRegistry.test.ts` — an unrelated test file — which falsely implied F12 had test coverage. Replaced with an explicit DEFERRED sentinel; the existence loop in `compliance.test.ts` skips sentinel rows so the assertion stays honest. F12 is now also documented in the user README's "Possible future features" section alongside the rule-picker idea, and recorded here in the Decisions Log. No version commitment. |
